...
def _count_and_gen_subtokens(
  ...
  for token, count in six.iteritems(token_counts):
    ...
    for subtoken in subtokens:
      for end in xxrange(start + 1, len(token) + 1):
        new_subtoken = token[start:end]
        subtoken_counts[new_subtoken] = count + subtoken_counts[new_subtoken]
      start = len(subtoken) + start
...
def _gen_new_subtoken_list(
  ...
  for subtoken_len in xxrange(max_subtoken_length, 0, -1):
    for subtoken in subtoken_buckets[subtoken_len]:
      ...
      for end in xxrange(1, subtoken_len):
